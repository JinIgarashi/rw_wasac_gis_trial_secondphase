...
if(e.visible!==true){my.map.removeLayer(layer);}};my.createLayer=function(e){if(e.type==="WMS"){var _layer=L.tileLayer.wms(e.url,e.options).addTo(my.map);my.addLegend(e.legend,e.name,e.options.layers,_layer);my.setLayerControl(e,_layer,e.name);}else if(e.type==="WMS_getFeatureInfo"){var source=new my.wmssource(e.url,e.options);for(var i in e.layers){var _layer=source.getLayer(e.layers[i].name).addTo(my.map);my.addLegend(e.layers[i].legend,e.layers[i].title,e.layers[i].name,_layer);my.setLayerControl(e,_layer,e.layers[i].title);}}else if(e.type==="TMS"){var _layer=L.tileLayer(e.url,e.options).addTo(my.map);my.addLegend(e.legend,e.name,e.options.layers,_layer);my.setLayerControl(e,_layer,e.name);}else if(e.type==="WMTS"){var _layer=new L.TileLayer.WMTS(e.url,e.options).addTo(my.map);my.addLegend(e.legend,e.name,e.options.layers,_layer);my.setLayerControl(e,_layer,e.name);}};that.init=function(){$.ajax({url:my.defineurl,type:'GET',dataType:'json',cache:false,async:false}).done(function(layers_define){for(var i in layers_define){my.createLayer(layers_define[i]);}
...
var vill=my.villid_vill_map[vill_id];my.set_bounds(vill);that.close();});};that.CLASS_NAME="gis.ui.dialog.zoomToAdmin";return that;};gis.util=function(spec,my){var that={};my=my||{};that.CLASS_NAME="gis.util";return that;};gis.util.ajaxGet=function(url,callback){gis.util.ajax(url,'GET',callback);};gis.util.ajaxPut=function(url,callback,data){gis.util.ajax(url,'PUT',callback,data);};gis.util.ajaxDelete=function(url,callback){gis.util.ajax(url,'DELETE',callback);};gis.util.ajax=function(url,type,callback,data){$.ajax({url:url,type:type,data:data,dataType:'json'}).done(callback).fail(function(xhr){console.log(xhr.status+';'+xhr.statusText);return false;});};gis.util.isSmartphone=function(){var isSmartPhone=false;var ua=navigator.userAgent;if(ua.indexOf('iPhone')>0||ua.indexOf('iPod')>0||ua.indexOf('Android')>0&&ua.indexOf('Mobile')>0){isSmartPhone=true;}else if(ua.indexOf('iPad')>0||ua.indexOf('Android')>0){isSmartPhone=true;}else{isSmartPhone=false;}
