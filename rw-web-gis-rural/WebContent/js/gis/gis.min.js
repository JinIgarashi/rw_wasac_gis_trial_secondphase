...
my.prov_dist_map[district.prov_id].push(district);my.distid_dist_map[district.dist_id]=district;}});};my.getSectors=function(){gis.util.ajaxGet('./rest/Sectors',function(json){if(json.code!==0){alert(json.message);return;}
...
my.dist_sector_map[sector.dist_id].push(sector);my.sectid_sect_map[sector.sect_id]=sector;}});};my.getCells=function(){gis.util.ajaxGet('./rest/Cells',function(json){if(json.code!==0){alert(json.message);return;}
...
my.sect_cell_map[cell.sect_id].push(cell);my.cellid_cell_map[cell.cell_id]=cell;}});};my.getVillages=function(){gis.util.ajaxGet('./rest/Villages',function(json){if(json.code!==0){alert(json.message);return;}
...
my.cell_vill_map[vill.cell_id].push(vill);my.villid_vill_map[vill.vill_id]=vill;}});};my.postCreate=function(){my.getProvinces();my.getDistricts();my.getSectors();my.getCells();my.getVillages();};my.beforeOpen=function(){$("#"+my.comboboxProvId).append($('<option>').html("Select Province").val(""));for(var prov_id in my.provid_prov_map){var province=my.provid_prov_map[prov_id];$("#"+my.comboboxProvId).append($('<option>').html(province.prov_name).val(province.prov_id));my.provid_prov_map[province.prov_id]=province;}
...
my.dist_wss_map[wss.dist_id].push(wss);my.wssid_wss_map[wss.wss_id]=wss;}});};my.postCreate=function(){my.getPos();my.getDistricts();my.getWSSs();};my.beforeOpen=function(){$("#"+my.comboboxPoId).append($('<option>').html("Select Private Operator").val(""));for(var po_id in my.poid_po_map){var po=my.poid_po_map[po_id];$("#"+my.comboboxPoId).append($('<option>').html(po.po_name).val(po.po_id));}
...
for(var wss_id in my.po_wss_map[po_id]){var wss=my.po_wss_map[po_id][wss_id];$("#"+my.comboboxWssId).append($('<option>').html(wss.wss_name).val(wss.wss_id));}});$("#"+my.comboboxDistId).change(function(){var dist_id=$(this).val();$("#"+my.comboboxWssId+" > option").remove();$("#"+my.comboboxWssId).append($('<option>').html("Select WSS").val(""));$("#"+my.comboboxPoId).val("");if(!my.dist_wss_map[dist_id]){return;}
...
var district=my.distid_dist_map[dist_id];my.set_bounds(district);});$("#"+my.comboboxWssId).change(function(){var wss_id=$(this).val();if(wss_id==""){return;}
var wss=my.wssid_wss_map[wss_id];my.set_bounds(wss);that.close();});};that.CLASS_NAME="gis.ui.dialog.zoomToWss";return that;};gis.ui.sidebar.zoomToAdmin=function(spec,my){my=my||{};var that=gis.ui.sidebar(spec,my);my.divid='slidebar-zoomToAdmin'
my.comboboxProvId='cmbProvId_'+my.divid;my.comboboxDistId='cmbDistId_'+my.divid;my.comboboxSectorId='cmbSectId_'+my.divid;my.comboboxCellId='cmbCellId_'+my.divid;my.comboboxVillId='cmbVillId_'+my.divid;my.provid_prov_map={};my.distid_dist_map={};my.sectid_sect_map={};my.cellid_cell_map={};my.villid_vill_map={};my.prov_dist_map={};my.dist_sector_map={};my.sect_cell_map={};my.cell_vill_map={};my.selected_bounds=null;my.getHtml=function(){var html="<label for='"+my.comboboxProvId+"'>Province</label>"+"<br>"+"<select id='"+my.comboboxProvId+"' class='gis-ui-sidebar-zoom-selectbox'></select>"+"<br>"+"<label for='"+my.comboboxDistId+"'>District</label>"+"<br>"+"<select id='"+my.comboboxDistId+"' class='gis-ui-sidebar-zoom-selectbox'></select>"+"<br>"+"<label for='"+my.comboboxSectorId+"'>Sector</label>"+"<br>"+"<select id='"+my.comboboxSectorId+"' class='gis-ui-sidebar-zoom-selectbox'></select>"+"<br>"+"<label for='"+my.comboboxCellId+"'>Cell</label>"+"<br>"+"<select id='"+my.comboboxCellId+"' class='gis-ui-sidebar-zoom-selectbox'></select>"+"<br>"+"<label for='"+my.comboboxVillId+"'>Village</label>"+"<br>"+"<select id='"+my.comboboxVillId+"' class='gis-ui-sidebar-zoom-selectbox'></select>";return html;};my.getContent=function(){return{id:my.divid,tab:'<i class="fa fa-sitemap"></i>',pane:my.getHtml(),title:'Zoom To Administrative',position:'top'};};my.getProvinces=function(){gis.util.ajaxGet('./rest/Provinces',function(json){if(json.code!==0){alert(json.message);return;}
...
var vill=my.villid_vill_map[vill_id];my.set_bounds(vill);});});};that.bindData=function(){my.getProvinces();my.getDistricts();my.getSectors();my.getCells();my.getVillages();};that.CLASS_NAME="gis.ui.sidebar.zoomToAdmin";return that;};gis.ui.control.coordinates=function(spec,my){my=my||{};var that=gis.ui.control(spec,my);my.options=spec.options||{position:"bottomright",decimals:6,labelTemplateLat:"Latitude: {y}",labelTemplateLng:"Longitude: {x}",};my.notRequireSP=spec.notRequireSP||true;that.init=function(){my.control=L.control.coordinates(my.options);};that.CLASS_NAME="gis.ui.control.coordinates";return that;};gis.ui.sidebar.legend=function(spec,my){my=my||{};var that=gis.ui.sidebar(spec,my);my.legends=spec.legends;my.getContent=function(){if(my.legends.length===0){return;}
var html="";for(var i in my.legends){var legend=my.legends[i];if(i>0){html+="<hr>";}
html+="<h2>"+legend.title+"</h2>";html+=legend.elements[0].html;}
return{id:'slidebar-legend',tab:'<i class="fa fa-list"></i>',pane:html,title:'LEGEND',position:'bottom'};};that.CLASS_NAME="gis.ui.sidebar.legend";return that;};gis.ui.control.locate=function(spec,my){my=my||{};var that=gis.ui.control(spec,my);that.init=function(){my.control=L.control.locate(my.options);};that.CLASS_NAME="gis.ui.control.locate";return that;};gis.ui.control.graphicScale=function(spec,my){my=my||{};var that=gis.ui.control(spec,my);my.options=spec.options||{fill:'hollow',showSubunits:true,labelPlacement:'top'};my.notRequireSP=spec.notRequireSP||true;that.init=function(){my.control=L.control.graphicScale(my.options);};that.CLASS_NAME="gis.ui.control.graphicScale";return that;};gis.ui.control.zoomToAreas=function(spec,my){my=my||{};var that=gis.ui.control(spec,my);that.init=function(){my.dialogWss=gis.ui.dialog.zoomToWss({divid:"dialogZoomToWss",map:my.map});my.dialogWss.create();my.dialogAdmin=gis.ui.dialog.zoomToAdmin({divid:"dialogZoomToAdmin",map:my.map});my.dialogAdmin.create();my.control=L.easyBar([L.easyButton('fa-tint',function(){my.dialogWss.open();},'Zoom To WSS'),L.easyButton('fa-sitemap',function(){my.dialogAdmin.open();},'Zoom To Administrative Boundary')]);};that.CLASS_NAME="gis.ui.control.zoomToAreas";return that;};gis.util=function(spec,my){var that={};my=my||{};that.CLASS_NAME="gis.util";return that;};gis.util.ajaxGet=function(url,callback){gis.util.ajax(url,'GET',callback);};gis.util.ajaxGetAsync=function(url,callback){gis.util.ajax(url,'GET',callback,null,false);};gis.util.ajaxPut=function(url,callback,data){gis.util.ajax(url,'PUT',callback,data);};gis.util.ajaxDelete=function(url,callback){gis.util.ajax(url,'DELETE',callback);};gis.util.ajax=function(url,type,callback,data,async){$.ajax({url:url,type:type,data:data,dataType:'json',async:async}).done(callback).fail(function(xhr){console.log(xhr.status+';'+xhr.statusText);return false;});};gis.util.isSmartphone=function(){var isSmartPhone=false;var ua=navigator.userAgent;if(ua.indexOf('iPhone')>0||ua.indexOf('iPod')>0||ua.indexOf('Android')>0&&ua.indexOf('Mobile')>0){isSmartPhone=true;}else if(ua.indexOf('iPad')>0||ua.indexOf('Android')>0){isSmartPhone=true;}else{isSmartPhone=false;}
return isSmartPhone;};gis.ui.sidebar.zoomToWss=function(spec,my){my=my||{};var that=gis.ui.sidebar(spec,my);my.divid='slidebar-zoomToWss'
my.comboboxPoId='cmbPoId_'+my.divid;my.comboboxDistId='cmbDistId_'+my.divid;my.comboboxWssId='cmbWssId_'+my.divid;my.poid_po_map={};my.distid_dist_map={};my.wssid_wss_map={};my.po_wss_map={};my.dist_wss_map={};my.getHtml=function(){var html="<label for='"+my.comboboxPoId+"'>Private Operator</label>"+"<br>"+"<select id='"+my.comboboxPoId+"' class='gis-ui-sidebar-zoom-selectbox'></select>"+"<br>"+"<label for='"+my.comboboxDistId+"'>District</label>"+"<br>"+"<select id='"+my.comboboxDistId+"' class='gis-ui-sidebar-zoom-selectbox'></select>"+"<br>"+"<label for='"+my.comboboxWssId+"'>WSS</label>"+"<br>"+"<select id='"+my.comboboxWssId+"' class='gis-ui-sidebar-zoom-selectbox'></select>";return html;};my.getContent=function(){return{id:my.divid,tab:'<i class="fa fa-tint"></i>',pane:my.getHtml(),title:'Zoom To WSS',position:'top'};};my.getPos=function(){gis.util.ajaxGet('./rest/POs',function(json){if(json.code!==0){alert(json.message);return;}
my.poid_po_map={};var pos=json.value;for(var i in pos){var po=pos[i];my.poid_po_map[po.po_id]=po;}
...
if(html===""){return;}
html="<table class='popup-table-wms-getfeatureinfo'>"+html+"</table>";this._map.openPopup(html,latlng);}});my.legends=[];my.addLegend=function(legend,title,layername,layer){if(legend){my.legends.push({title:title,name:layername,layer:layer,elements:legend.elements});}};my.setLayerControl=function(e,layer,name){if(e.isBaseLayer&&e.isBaseLayer===true){my.baseMaps.push({title:name,layer:layer,icon:e.icon});}else{if(!e.group){my.overlays[name]=layer;}else{if(!my.overlays[e.group]){my.overlays[e.group]={};}
my.overlays[e.group][name]=layer;}}
if(e.visible===true){my.map.addLayer(layer);}else{my.map.removeLayer(layer);}};my.createLayer=function(e){if(e.type==="WMS"){var _layer=L.tileLayer.wms(e.url,e.options);my.addLegend(e.legend,e.name,e.options.layers,_layer);my.setLayerControl(e,_layer,e.name);}else if(e.type==="WMS_getFeatureInfo"){var source=new my.wmssource(e.url,e.options);for(var i in e.layers){var _layer=source.getLayer(e.layers[i].name);my.addLegend(e.layers[i].legend,e.layers[i].title,e.layers[i].name,_layer);my.setLayerControl(e,_layer,e.layers[i].title);}}else if(e.type==="TMS"){var _layer=L.tileLayer(e.url,e.options);my.addLegend(e.legend,e.name,e.options.layers,_layer);my.setLayerControl(e,_layer,e.name);}else if(e.type==="WMTS"){var _layer=new L.TileLayer.WMTS(e.url,e.options);my.addLegend(e.legend,e.name,e.options.layers,_layer);my.setLayerControl(e,_layer,e.name);}else if(e.type==="GeoJSON"){gis.util.ajaxGetAsync(e.url,function(geojson){var options={onEachFeature:function(feature,layer){if(!feature.properties){return;}
...
var _layer=L.geoJSON(geojson,options);markers=L.markerClusterGroup();markers.addLayer(_layer);my.addLegend(e.legend,e.name,e.name,markers);my.setLayerControl(e,markers,e.name);});}};that.init=function(){$.ajax({url:my.defineurl,type:'GET',dataType:'json',cache:false,async:false}).done(function(layers_define){for(var i in layers_define){my.createLayer(layers_define[i]);}
...
L.control.groupedLayers({},my.overlays,{exclusiveGroups:["Area"],groupCheckboxes:true}).addTo(my.map);});};that.CLASS_NAME="gis.ui.layerLoader";return that;};gis.ui.controlLoader=function(spec,my){var that={};my=my||{};my.map=spec.map;my.defineurl=spec.defineurl;that.init=function(){$.ajax({url:my.defineurl,type:'GET',dataType:'json',cache:false,async:true}).done(function(ctrls_define){var settings={map:my.map};for(var i in ctrls_define){for(var j in ctrls_define[i].settings){settings[j]=ctrls_define[i].settings[j];}
var ctrl=gis.ui.control[ctrls_define[i].ctrl](settings);ctrl.init();ctrl.add();}});};that.CLASS_NAME="gis.ui.controlLoader";return that;};gis.ui.control.geocoder=function(spec,my){my=my||{};var that=gis.ui.control(spec,my);that.init=function(){my.control=L.Control.geocoder(my.options);};that.CLASS_NAME="gis.ui.control.geocoder";return that;};gis.ui.sidebar.print=function(spec,my){my=my||{};var that=gis.ui.sidebar(spec,my);my.elementsToHide=spec.elementsToHide||["a","button",".leaflet-small-widget",".leaflet-control-coordinates",".leaflet-control-attribution",".leaflet-iconLayers-layer",".leaflet-bar",".leaflet-control-geocoder-icon",".leaflet-sidebar"];my.addCss=function(){var css=document.createElement("style");css.type="text/css";css.innerHTML="._epHidden{ \
   display:none!important; \
  }";document.body.appendChild(css);};my.getContent=function(){my.addCss();return{id:'sideber-print',tab:'<i class="fa fa-print"></i>',button:function(event){if(my.elementsToHide){var htmlElementsToHide=document.querySelectorAll(my.elementsToHide);for(var i=0;i<htmlElementsToHide.length;i++){htmlElementsToHide[i].className=htmlElementsToHide[i].className+' _epHidden';}}
my.map.fire("beforePrint");window.print();my.map.fire("afterPrint");if(my.elementsToHide){var htmlElementsToHide=document.querySelectorAll(my.elementsToHide);for(var i=0;i<htmlElementsToHide.length;i++){htmlElementsToHide[i].className=htmlElementsToHide[i].className.replace(' _epHidden','');}}}};};that.CLASS_NAME="gis.ui.sidebar.print";return that;};