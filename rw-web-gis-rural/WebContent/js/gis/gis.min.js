...
if(e.visible!==true){my.map.removeLayer(layer);}};my.createLayer=function(e){if(e.type==="WMS"){var _layer=L.tileLayer.wms(e.url,e.options).addTo(my.map);my.addLegend(e.legend,e.name,e.options.layers,_layer);my.setLayerControl(e,_layer,e.name);}else if(e.type==="WMS_getFeatureInfo"){var source=new my.wmssource(e.url,e.options);for(var i in e.layers){var _layer=source.getLayer(e.layers[i].name).addTo(my.map);my.addLegend(e.layers[i].legend,e.layers[i].title,e.layers[i].name,_layer);my.setLayerControl(e,_layer,e.layers[i].title);}}else if(e.type==="TMS"){var _layer=L.tileLayer(e.url,e.options).addTo(my.map);my.addLegend(e.legend,e.name,e.options.layers,_layer);my.setLayerControl(e,_layer,e.name);}};that.init=function(){$.ajax({url:my.defineurl,type:'GET',dataType:'json',cache:false,async:false}).done(function(layers_define){for(var i in layers_define){my.createLayer(layers_define[i]);}
