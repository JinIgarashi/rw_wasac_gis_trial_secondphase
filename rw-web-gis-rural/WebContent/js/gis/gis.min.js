...
if(e.visible===true){my.map.addLayer(layer);}else{my.map.removeLayer(layer);}};my.createLayer=function(e){if(e.type==="WMS"){var _layer=L.tileLayer.wms(e.url,e.options);my.setLayerControl(e,_layer,e.name);}else if(e.type==="WMS_getFeatureInfo"){var source=new my.wmssource(e.url,e.options);for(var i in e.layers){var _layer=source.getLayer(e.layers[i].name);my.setLayerControl(e,_layer,e.layers[i].title);}}else if(e.type==="TMS"){var _layer=L.tileLayer(e.url,e.options);my.setLayerControl(e,_layer,e.name);}else if(e.type==="WMTS"){var _layer=new L.TileLayer.WMTS(e.url,e.options);my.setLayerControl(e,_layer,e.name);}else if(e.type==="GeoJSON"){var options={onEachFeature:function(feature,layer){if(!feature.properties){return;}
...
var _layer=new L.GeoJSON.AJAX(e.url,options);if(!my.mcgroup){my.mcgroup=L.markerClusterGroup();my.mcgroup.addTo(my.map);}
var markers=L.featureGroup.subGroup(my.mcgroup);markers.addLayer(_layer);markers.addTo(my.map);my.setLayerControl(e,markers,e.name);}};that.init=function(){gis.util.ajaxGetAsync(my.defineurl,function(layers_define){my.layerControl=L.Control.styledLayerControl({},my.overlays,{container_width:"300px",group_maxHeight:"90%",exclusive:false,group_togglers:{show:true,labelAll:'Select All',labelNone:'Deselect All'},}).addTo(my.map);L.DomEvent.on(my.layerControl._container,'mouseenter',function(){my.map.scrollWheelZoom.disable();});L.DomEvent.on(my.layerControl._container,'mouseleave',function(){my.map.scrollWheelZoom.enable();});for(var i in layers_define){my.createLayer(layers_define[i]);}
